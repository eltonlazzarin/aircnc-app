...
export default function Dashboard() {
    ...
    const socket = useMemo(() => socketio('http://192.168.0.104:3333', {
        query: { user_id }
    }), [user_id]);
    ...
    return (
        <>
            <ul className="nofitications">
                {requests.map(request => (
                    <li key={request._id}>
                        <p>
                            <strong>{request.user.email}</strong> is requesting a reservation in <strong>{request.spot.company}</strong> to date: <strong>{request.date}</strong>
                        </p>
